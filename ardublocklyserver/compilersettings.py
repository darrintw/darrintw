...
class ServerCompilerSettings(object):
    ...
    # Serial monitor baud rate options, static content
    __baud_rate_options = {'2400': '2400 baud',
                           '4800': '4800 baud',
                           '9600': '9600 baud',
                           '19200': '19200 baud',
                           '57600': '57600 baud',
    ...
    # Serial monitor end of line options, static content
    __end_of_line_options = {'none': 'none',
                             'nl': 'NL(newline)',
    ...
    def set_compiler_dir_default(self):
        self.__compiler_dir = 'arduino\\arduino_debug.exe'
    ...
    def set_compiler_dir_from_file(self, new_compiler_dir):
        ...
        if os.path.exists(new_compiler_dir):
            self.__compiler_dir = new_compiler_dir
        elif os.path.exists(os.path.dirname(self.__settings_path) + '\\' + new_compiler_dir):
            self.__compiler_dir = new_compiler_dir
        else:
    ...
    def set_sketch_name_default(self):
        self.__sketch_name = 'Sketch_Name'
    ...
    def set_sketch_dir_default(self):
        ...
        self.__sketch_dir = 'ArdublocklySketch'
    ...
    #
    #  Example Directory accessors
    #
    def get_examples_dir(self):
        return self.__examples_dir

    def set_examples_dir(self, new_examples_dir):
        """Set the example directory, which must be a folder."""
        if os.path.isdir(new_examples_dir):
            self.__examples_dir = new_examples_dir
            print('Example directory set to:\n\t%s' % self.__examples_dir)
            self.save_settings()
        else:
            print('The provided example directory is not valid !!!'
                  '\n\t%s' % new_examples_dir)
            if self.__examples_dir:
                print('Previous example directory maintained:\n\t%s' %
                      self.__examples_dir)
            else:
                self.set_examples_dir_default()
                print('Default example directory set:\n\t%s' %
                      self.__examples_dir)
                self.save_settings()

    examples_dir = property(get_examples_dir, set_examples_dir)

    def set_examples_dir_default(self):
        """Example default location the same as the settings file location."""
        # self.__examples_dir = os.path.dirname(self.__settings_path)
        self.__examples_dir = 'examples'

    def set_examples_dir_from_file(self, new_examples_dir):
        """Set the example directory from settings file, must be a folder."""
        if os.path.isdir(new_examples_dir):
            self.__examples_dir = new_examples_dir
        elif os.path.isdir(os.path.dirname(self.__settings_path) + '\\' + new_examples_dir):
            self.__examples_dir = new_examples_dir
        else:
            print('Settings file example directory is not valid:'
                  '\n\t%s' % new_examples_dir)
            self.set_examples_dir_default()
            print('Default Example directory set:\n\t%s' % self.__examples_dir)
    ...
    def set_load_ide_default(self):
        self.__load_ide_option = \
            sorted(self.__ide_load_options.keys())[1]
    ...
    def get_baud_rate(self):
        return self.__baud_rate_option

    def set_baud_rate(self, new_baud_rate_option):
        if new_baud_rate_option in self.__baud_rate_options:
            self.__baud_rate_option = new_baud_rate_option
            print('Baud Rate options set to:\n\t%s' %
                  self.__baud_rate_options[self.__baud_rate_option])
            self.save_settings()
        else:
            print('The provided "Baud Rate option" is not valid !!!'
                  '\n\t%s' % new_baud_rate_option)
            if self.__baud_rate_option:
                print('Previous "Baud Rate option" maintained:\n\t%s' %
                      self.__baud_rate_options[self.__baud_rate_option])
            else:
                self.set_baud_rate_default()
                print('Default "Baud Rate option" set:\n\t%s' %
                      self.__baud_rate_options[self.__baud_rate_option])
                self.save_settings()

    baud_rate_option = property(get_baud_rate, set_baud_rate)

    def set_baud_rate_default(self):
        self.__baud_rate_option = \
            sorted(self.__baud_rate_options.keys())[5]

    def set_baud_rate_from_file(self, new_baud_rate_option):
        if new_baud_rate_option in self.__baud_rate_options:
            self.__baud_rate_option = new_baud_rate_option
        else:
            print('Settings file "Baud Rate option" is not valid:'
                  '\n\t%s' % new_baud_rate_option)
            self.set_baud_rate_default()
            print('Default "Baud Rate option" set:\n\t%s' %
                  self.__baud_rate_option)
    ...
    def save_settings(self):
        ...
        settings_parser.set('Arduino_Sketch', 'examples_directory', '%s' % self.examples_dir)
        # Ardublockly section
    ...
    def get_settings_file_data(self):
        ...
        try:
            ...
            settings_dict['examples_directory'] = \
                settings_parser.get('Arduino_Sketch', 'examples_directory')
