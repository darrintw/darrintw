...
def launch_server(ip='localhost', port=8007, document_root_=''):
    ...
    :param port: Port to serve, default 8007.
    :param document_root_: Path to be the server document root, default cwd.
...
def static_ardublockly(file_path):
    ...
    """
    return static_file(file_path, root=os.path.join(document_root, 'ardublockly'))
...
#
# Get example list only get.
#
@app.get('/exampleslist')
def handler_examples_get_all():
    """Handle the GET all settings requests.

    :return: JSON string with the formed response.
    """
    response_dict = {
        'response_type': 'exampleslist',
        'response_state': 'full_response',
        'success': True,
        'exampleslist_type': 'all',
        'exampleslist': {
            'list': actions.get_examples_files(),
            'path': actions.get_examples_path()
        }
    }
